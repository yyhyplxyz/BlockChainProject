...
type LoanChange struct{
	...
	Originalnumber float64 `json:"Originalnumber"`
...
func LoanEnroll(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	debtor_ID := args[2]
	number := args[3]
	thenum, err := strconv.ParseFloat(number, 64)

	if Loan_ID == "" || creditor_ID == "" || debtor_ID == "" {
		return shim.Error("invalid args")
	}
	...
	debtoruserBytes, err := stub.GetState(constructUserKey(debtor_ID))
	if err != nil || len(debtoruserBytes) == 0{
		return shim.Error("debtor user not found")
	}
